...
if (!isset($_SESSION['control_test'])) {
    die(t('error_session_reset'));
}

$test = $_SESSION['control_test'];
...
$grade = $correct >= 13 ? 5 : ($correct >= 10 ? 4 : ($correct >= 7 ? 3 : 2));

try {
    $stmt = $pdo->prepare("INSERT INTO control_test_results
        (subject_id, test_id, class_id, student_name, student_lastname, grade, correct_answers_count, answers)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([
        $test['subject_id'],
        $test['test_id'],
            ...
            <div class="score-circle">
                <svg class="score-svg" viewBox="0 0 160 160">
                    ...
                    <circle class="score-circle-progress" cx="80" cy="80" r="70"
                        style="stroke-dashoffset: <?= 440 - (440 * ($correct / $total_questions)) ?>"></circle>
                </svg>
                <div class="score-text"><?= $grade ?></div>
            </div>
            <div class="score-details">
                <div class="correct-answers"><?= $correct ?> <?= t('label_correct') ?></div>
                <div class="total-questions"><?= t('label_total_questions', ['count' => $total_questions]) ?></div>
            </div>
        </div>
        <div class="mistakes-section">
            <h2 class="section-title">
                <span class="material-icons">error_outline</span>
                <?= t('section_title_mistakes', ['count' => $total_questions - $correct]) ?>
            </h2>
            <ul class="mistakes-list">
                <?php foreach ($details as $d): ?>
                    <?php if (!$d['is_correct']): ?>
                        <li class="mistake-item">
                            <div class="question-text"><?= t('label_question', ['number' => $d['question'] + 1]) ?></div>
            ...
            <a href="control_works.php" class="home-button">
